...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\farerilab\\Downloads\\GamblingAdviceTask\\GamblingAdviceTask3_lastrun.py',
...
ask = visual.TextStim(win=win, name='ask',
    ...
    languageStyle='LTR',
    depth=-3.0);
getAdvice = visual.TextStim(win=win, name='getAdvice',
    text='default text',
    font='Arial',
    pos=[0,0], height=1.0, wrapWidth=None, ori=0, 
    color='white', colorSpace='rgb', opacity=1, 
...
for thisTrial_2 in trials_2:
    ...
    for thisTrial in trials:
        ...
        ask.setFont('Arial')
        ask.setHeight(0.06)
        getAdvice.setColor('white', colorSpace='rgb')
        getAdvice.setPos((0, 0))
        getAdvice.setText(text + AdviceVar1)
        getAdvice.setFont('Arial')
        getAdvice.setHeight(0.1)
        textforcode.setColor([-1.000,-1.000,-1.000], colorSpace='rgb')
        textforcode.setPos((0, 0))
        textforcode.setText(GambleAdviceNum)
        textforcode.setFont('Arial')
        textforcode.setHeight(0.01)
        # keep track of which components have finished
        askAdComponents = [response, yesNo, ask, getAdvice, textforcode]
        ...
        while continueRoutine:
            ...
            # *getAdvice* updates
            if (response.keys == 'y') and getAdvice.status == NOT_STARTED:
                # keep track of start time/frame for later
                getAdvice.tStart = t  # not accounting for scr refresh
                getAdvice.frameNStart = frameN  # exact frame index
                win.timeOnFlip(getAdvice, 'tStartRefresh')  # time at next scr refresh
                getAdvice.setAutoDraw(True)
            if getAdvice.status == STARTED and t >= (getAdvice.tStart + 2):
                # keep track of stop time/frame for later
                getAdvice.tStop = t  # not accounting for scr refresh
                getAdvice.frameNStop = frameN  # exact frame index
                win.timeOnFlip(getAdvice, 'tStopRefresh')  # time at next scr refresh
                getAdvice.setAutoDraw(False)
            
            # *textforcode* updates
            if (response.keys == 'y') and textforcode.status == NOT_STARTED:
                # keep track of start time/frame for later
        ...
        trials.addData('getAdvice.started', getAdvice.tStartRefresh)
        trials.addData('getAdvice.stopped', getAdvice.tStopRefresh)
